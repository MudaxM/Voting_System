...
<head>
    ...
    <style>
        ...
        .candidate-option.selected .select-indicator {
            background-color: var(--primary);
            border-color: var(--primary);
            color: white;
        }

        .candidate-option.selected .select-indicator::after {
            content: '\f00c';
...
<body>
    ...
    <div class="dashboard">
        <div class="container">
            ...
            <div class="dashboard-header">
                <div class="user-info">
                    ...
                    <p>
                        ...
                        <i class="fas fa-graduation-cap"></i> <?php echo htmlspecialchars($user['department']); ?> -
            ...
            <div class="alert alert-info selection-tip" style="margin-bottom: 2rem; background: #ebf8ff; border-left: 4px solid var(--info);">
                <i class="fas fa-info-circle" style="color: var(--info);"></i>
                <div>
                    <strong>Pro Tip:</strong> Click or tap anywhere on a candidate's card to select them. A checkmark will appear to confirm your choice.
                </div>
            </div>
            ...
            <form method="POST" action="" id="votingForm">
                        ...
                        <div class="position-card">
                            <div class="position-header">
                                <h3>
                                    ...
                                    <span style="font-size: 0.9rem; opacity: 0.9;">(Step <?php echo $step; ?> of
                                        <?php echo count($positions); ?>)</span>
                                </h3>
                            </div>
                                        ...
                                        <?php foreach ($position['candidates'] as $candidate): ?>
                                            <label class="candidate-option" data-candidate-id="<?php echo $candidate['id']; ?>">
                                                <input type="radio" name="votes[<?php echo $position['id']; ?>]"
                                                    value="<?php echo $candidate['id']; ?>" class="candidate-radio"
                                                    data-position="<?php echo $position['id']; ?>" required>
                                                <div class="select-indicator"></div>

                                                <?php
                                                $photo_filename = !empty($candidate['photo']) ? htmlspecialchars($candidate['photo']) : 'default.jpg';
                                                $photo_path = (strpos($photo_filename, 'http') === 0) ? $photo_filename : SITE_URL . 'uploads/candidates/' . $photo_filename;
                                                ?>
                                                ...
                                                <div class="candidate-details">
                                                    ...
                                                    <div class="candidate-info">
                                                        <span><i class="fas fa-graduation-cap"></i>
                                                            <?php echo htmlspecialchars($candidate['department']); ?></span>
                                                        <span><i class="fas fa-id-badge"></i> Year
                                                            <?php echo $candidate['year']; ?></span>
                                                    </div>

                                                    <?php if (!empty($candidate['manifesto'])): ?>
                                                        <div class="candidate-manifesto">
                                                            <h5><i class="fas fa-bullhorn"></i> Manifesto Preview</h5>
                                                            <p><?php echo nl2br(htmlspecialchars(substr($candidate['manifesto'], 0, 160))); ?>...
                                                            </p>
                                                            <span
                                                                style="color: var(--primary); font-weight: 600; font-size: 0.85rem; display: block; margin-top: 10px;">
                                                                <i class="fas fa-plus-circle"></i> Click card to select
                                                            </span>
                                                        </div>
                                                    <?php endif; ?>
                                                </div>
                    ...
                    <div class="confirmation-step">
                        ...
                        <div class="alert alert-info" style="margin: 2rem 0;">
                            ...
                            <div>
                                ...
                                <p>Once you submit your vote, it cannot be changed. Please make sure your selections are
                                    correct.</p>
                    ...
                    <div class="confirmation-step">
                        ...
                        <div class="form-group" style="text-align: center;">
                            <div class="form-check" style="display: inline-block;">
                                <input type="checkbox" id="confirmSubmission" name="confirm_submission"
                ...
                <div class="step-navigation">
                    <button type="button" class="btn btn-outline" id="prevBtn" onclick="prevStep()"
                        style="display: none;">
        ...
        function nextStep() {
            ...
            if (currentStep <= positionsData.length) {
                ...
                if (!selectedCandidate) {
                    const cards = document.querySelectorAll(`#step${currentStep} .candidate-option`);
                    cards.forEach(card => {
                    ...
                    // Add a temporary warning message
                    let warning = document.querySelector('.selection-warning');
                    if (!warning) {
                        warning = document.createElement('div');
                        warning.className = 'selection-warning alert alert-error';
                        warning.innerHTML = '<i class="fas fa-hand-pointer"></i> Please tap on a candidate card to select them before proceeding.';
                        warning.style.marginTop = '1rem';
                        document.querySelector(`#step${currentStep} .position-body`).appendChild(warning);
